...
AS $$
BEGIN
    INSERT INTO tb_conta (cod_cliente, saldo, cod_tipo_conta) VALUES (p_cod_cli, p_saldo, p_cod_tipo_conta);
    RETURN TRUE;
...
DECLARE
    ...
    v_resultado BOOLEAN;
BEGIN
    SELECT fn_abrir_conta (v_cod_cli, v_saldo, v_cod_tipo_conta) INTO v_resultado;
...
BEGIN
    ...
    SELECT saldo INTO v_saldo_resultante FROM tb_conta c WHERE c.cod_cliente = p_cod_cliente AND c.cod_conta = p_cod_conta;
...
BEGIN
    ...
    IF v_saldo_remetente >= p_valor_transferencia AND v_saldo_destinatario + p_valor_transferencia >= 0 THEN
